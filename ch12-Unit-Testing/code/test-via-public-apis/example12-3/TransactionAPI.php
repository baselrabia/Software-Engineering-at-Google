...
final class TransactionAPI
{
    ...
    public function saveToDatabase(Transaction $transaction): void
    {
        ...
        $this->database->process($transaction);
    }

    public function setAccountBalance(Account $account): void {
        $this->database->setAccount($account);
    }

    public function getAccountBalance(string $accountName): float|int {
        return $this->database->getAccount($accountName)->getBalance();
    }
